...
// TODO:
// - use env var instead of project file to store API token
// - use config instead of code for bundled bots

object BotRunner extends Shutdownable {
  val system = ActorSystem("tobyks-system")
  val eventBus = new MessageEventBus
  val slackBot = system.actorOf(Props(classOf[SlackBotActor], new ExampleBotsBundle(), eventBus, this, None), "tobyks")

  var botInfo: Option[BotInfo] = None

  def main(args: Array[String]) {
    println("tobyks alive")
  ...
  class ExampleBotsBundle() extends BotModules {
    override def registerModules(context: ActorContext, websocketClient: ActorRef) = {
      ...
      context.actorOf(Props(classOf[GithubGardenerBot], eventBus), "GithubGardenerBot")
    }
  }
}
